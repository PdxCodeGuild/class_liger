...
form.addEventListener('submit', function(event){
    ...
    if(arrayNumber.length === 4){
        if(+arrayNumber[1] === 0){
            phraseResult.innerHTML = ones[arrayNumber[0]]
             + ' thousand ' + " " + ones[+([arrayNumber[2],arrayNumber[3]]).join("")];
        }
        else if(+arrayNumber[1] < 2){
            phraseResult.innerHTML = ones[arrayNumber[0]]
             + ' thousand ' + ones[arrayNumber[1]]
    ...
    if(arrayNumber.length === 5){
        if(+arrayNumber[1] === 0){
            phraseResult.innerHTML = ' ten thousand ' + ones[arrayNumber[2]] + ' hundred' + " " + ones[+([arrayNumber[3],arrayNumber[4]]).join("")];
        }
